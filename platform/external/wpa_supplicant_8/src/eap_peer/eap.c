				    ...
				    size_t max_len, size_t *imsi_len)
{
	...
	if (mnc_len < 0) {
		wpa_printf(MSG_WARNING, "Get MNC lenght failed");
...
#endif /* defined(PCSC_FUNCS) || defined(PCSC_FUNCS_SEMC)*/
...
struct wpabuf * eap_sm_buildIdentity(struct eap_sm *sm, int id, int encrypted)
{
	...
	if(check_identity_format() == IMSI) {	/* MTD-Connectivity-FY-SupportSwisscomEAP-SIM */
		if (config->pcsc && identity_len > 7) {
			if (eap_sm_append_3gpp_realm(sm, data, sizeof(data), &identity_len) < 0) {
				wpa_printf(MSG_WARNING, "Could not add realm to SIM identity");
				return -1;
			}
			wpa_hexdump_ascii(MSG_DEBUG, "IMSI + realm", (u8 *) data, identity_len);
			wpa_printf(MSG_WARNING, "EAP: buildIdentity: identity "
			   "Printf IMSI + realm : ");
			wpa_printf(MSG_WARNING, "%s", data);
		}
	}
	...
	wpabuf_put_data(resp, data, identity_len);

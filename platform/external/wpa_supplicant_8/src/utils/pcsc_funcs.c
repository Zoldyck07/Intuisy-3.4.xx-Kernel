...
int scard_get_mnc_len(struct scard_data *scard)
{
	...
	if (scard_select_file(scard, SCARD_FILE_GSM_EF_AD, buf, &blen)) {
		wpa_printf(MSG_WARNING, "SCARD: select file failed");
		return -1;
	}
	if (blen < 4) {
	...
	if (scard->sim_type == SCARD_GSM_SIM) {
		...
		if (scard_parse_fsp_templ(buf, blen, NULL, &file_size)) {
			wpa_printf(MSG_WARNING, "SCARD: parse file failed");
			return -3;
		}
	}
	if (file_size == 3) {
		wpa_printf(MSG_WARNING, "SCARD: MNC length not available");
		return -7;
	}
	if (file_size < 4 || file_size > (int) sizeof(buf)) {
		wpa_printf(MSG_WARNING, "SCARD: invalid file length=%ld",
			   (long) file_size);
		return -4;
	}

	if (scard_read_file(scard, buf, file_size)) {
		wpa_printf(MSG_WARNING, "SCARD: read file failed");
		return -5;
	}
	buf[3] = buf[3] & 0x0f; /* upper nibble reserved for future use  */
	if (buf[3] < 2 || buf[3] > 3) {
		wpa_printf(MSG_WARNING, "SCARD: invalid MNC length=%ld",
			   (long) buf[3]);
		return -6;
	}
	wpa_printf(MSG_WARNING, "SCARD: MNC length=%ld", (long) buf[3]);
	return buf[3];
}
